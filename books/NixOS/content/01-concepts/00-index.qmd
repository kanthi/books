...
| This part | Next part |
|-----------|-----------|
| Language + store + flakes literacy | **Nix on Linux** (`02-nix-on-linux/`) — Fedora 44+ dual-stack bridge (recommended) |
| Disposable experiments on any Nix install | Then **NixOS host** — machine as code, generations, modules, boot |
...
Before starting **Nix on Linux** (or NixOS host, if you skip the bridge), confirm:
...
|-----------------|-------|
| **Fedora / foreign distro daily driver** | Part **Nix on Linux** (`02-nix-on-linux`) — official Fedora Nix, HM standalone, dual-stack |
| Own a machine as code | Part **NixOS host** (`03-nixos-host`) |
