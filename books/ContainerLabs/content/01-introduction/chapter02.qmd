...
|-----------|-------------|
| **Operating System** | Linux (Ubuntu 22.04+/24.04, Debian 12+, Fedora current, RHEL 9+ / Rocky/Alma 9+), macOS 13+, Windows 11 with WSL2 |
| **CPU** | 2 cores (4+ recommended) |
| **RAM** | 4GB (8GB+ recommended) |
| **Storage** | 20GB free space (50GB+ recommended) |
| **Network** | Internet connection for image downloads |
| **Container engine** | Docker Engine (with Compose v2 plugin) or a compatible runtime; check current ContainerLab docs for Podman support |
...
# Verify Compose v2 (plugin form — not legacy docker-compose v1)
docker compose version
```

> **Compose note (2026):** use `docker compose …` (v2 plugin). The standalone `docker-compose` (v1 Python) binary is legacy; avoid it in new labs and docs.

#### Linux (Fedora / RHEL 9+ / Rocky / Alma)

```bash
# Fedora often has docker-ce from Docker’s repo or moby-engine from Fedora —
# follow current Docker docs for your exact distro version.

# RHEL 9-compatible example using dnf + Docker CE repo:
...
> CentOS Linux 7 is end-of-life; do not target it for new lab hosts. Use RHEL 9+, Rocky/Alma 9+, or Fedora.
...
# Prefer the install script above so you always get a current release.
# To pin a specific version, check GitHub Releases and substitute VERSION:
#   https://github.com/srl-labs/containerlab/releases
# Example pattern (replace VERSION and ARCH after checking current assets):
#   VER=0.XX.Y
#   curl -sL "https://github.com/srl-labs/containerlab/releases/download/v${VER}/containerlab_${VER}_linux_amd64.tar.gz" \
#     | sudo tar -xzC /usr/local/bin/ containerlab
```

#### Manual Installation

```bash
# Look up the latest (or desired) tag on GitHub Releases first — do not hardcode outdated versions.
# Then download the asset matching your OS/arch, extract, and install to PATH:
#   tar -xzf containerlab_*_linux_amd64.tar.gz
#   sudo mv containerlab /usr/local/bin/
#   sudo chmod +x /usr/local/bin/containerlab

# Quick path: official installer
bash -c "$(curl -sL https://get.containerlab.dev)"
```
...
Expected output includes a version banner and fields such as `version`, `commit`, `date`, and `source`. Exact version numbers change frequently—confirm you are on a supported recent release via [containerlab.dev](https://containerlab.dev) and the project release notes rather than matching a year-old pin.
