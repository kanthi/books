...
Deploy the runtime closure. Do not deploy gcc because you compiled on this machine.
...
References are one hop. Requisites are the transitive closure. For `rg` they are often almost the same. For a desktop environment they are not.

### Case 4: Why gcc is missing from runtime

```bash
nix-store -q --deriver $(nix path-info nixpkgs#hello)
nix-store -q --references $(nix-store -q --deriver $(nix path-info nixpkgs#hello))
```
...
4. Intentionally `scp` only the `hello` store directory to a tmp dir and try to run it with `./bin/hello`. Then explain the interpreter error using the RPATH chapter.
