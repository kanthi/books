...
This is **not** content-addressed store paths. Paths stay input-addressed and distinct. Only file *contents* are shared.

It is also not compression. `du` after `--optimise` drops; `nix path-info -S` (logical size) may look similar because it counts each path separately.
...
On a fresh VM the number may be small. On a desk that has compiled three gcc versions it can be gigabytes.
...
Pick a file that is likely duplicated (a license):
...
That pair is the boring monthly (or the automatic timers).
...
The other trap is running `--optimise` while blaming it for a "missing" file. Optimise never deletes a path. If a file vanished, GC or a failed disk did it.
