...
Project compilers still belong in `devShell` / direnv (part 3). This chapter is the **user** profile and one-shot PATH.
...
`readlink -f $(which jq)` is a store path.
...
First search can be slow (eval). After that, 26.05 is cached.
...
The other trap is `nix profile install` of `go`, `nodejs`, `rustc`. The next repo wants another version. Part 3: devShell.
...
1. `nix shell github:NixOS/nixpkgs/nixos-26.05#neofetch --command neofetch` (or `fastfetch` if you prefer).
...
4. `nix profile list` and delete anything that is a compiler.
