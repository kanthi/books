...
Same `go` attr in both. `direnv` + `use flake` (part 3). `CGO_ENABLED=0` in `shellHook` if the package sets it.
...
The other trap is `GOTOOLCHAIN=auto` (Go default) downloading `go1.23.x` on first `go build`. `GOTOOLCHAIN=local`.
