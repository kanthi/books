...
Remote builder (`ssh-ng` to an ARM box) is **native**. Prefer it for Go, kernels, and NixOS tests. Cross is fine for small C.
...
You cannot run it on x86_64 without qemu-user.
...
`$CC` is the cross compiler. Do not hardcode `gcc`.
...
Tests that execute the binary must be off when crossing (`doCheck` false).

### Case 4: Flake overlay
  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      packages.x86_64-linux.desk-arm =
        pkgs.pkgsCross.aarch64-multiplatform.callPackage ./desk.nix { };
    };
}
```
...
`nixos-rebuild` / `nixosTest` for aarch64: remote `aarch64-linux` builder with `kvm`. Cross-compiling a full NixOS closure is possible and painful. The remote-builders chapter is the boring path for OS images.

## The trap

The trap is **running `./result/bin/desk-arm` on the laptop** and calling the compiler broken. `file` first. `qemu-user` if you must execute; a builder if you must test for real.
...
4. `nix eval --expr 'builtins.attrNames (import <nixpkgs> {}).pkgsCross' | tr ' ' '\n' | head`
