...
NixOS owns `services.vault-agent` (or a `systemd.services` wrapper if the module name differs on your pin — `nixos-option services.vault-agent`). The **role-id** can live in the store. The **secret-id** cannot.
