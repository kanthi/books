...
If the YAML value is a raw password, `EnvironmentFile` is the wrong shape (that file is `password=…` format). Either store a dotenv in sops (`DESK_DB=…`) or `LoadCredential` / `bind` the file as the password file the app already understands.
...
- Units consume `.path`, never decrypted strings in Nix.
