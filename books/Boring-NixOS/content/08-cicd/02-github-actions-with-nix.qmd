...
`CACHIX_AUTH_TOKEN` is a repository secret. Pull requests from forks should **not** receive a write token (GitHub's default is correct: `pull_request` from forks cannot read org secrets).
...
- One install-Nix action, then `nix flake check` and `nix build`.
- Same `flake.lock` as developers.
- Push to the team cache from trusted branches.
- Pin action versions.
...
4. Time `nix flake check` locally versus in CI. If CI is 10× slower, you are compiling; fix substituters.
