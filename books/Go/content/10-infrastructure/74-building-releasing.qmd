...
## Why `CGO_ENABLED=0` is part of a release
...
`CGO_ENABLED=0` also selects Go's own DNS resolver and `os/user` path instead of libc. That is what you want for `scratch` / distroless images and for a CLI you copy onto someone else's laptop. Turn cgo on only when a C library is irreplaceable. The full kernel-vs-libc story is in [Go as a Systems Language](../14-systems-programming/140a-go-as-systems-language.qmd).
...
- [Containerization](70-docker-multistage.qmd)
