...
**Tooling baseline (mid-2026):** examples assume a recent Go toolchain (Go 1.26-class: `WaitGroup.Go`, modern `sync/atomic` types, `go test -race`). Use `go mod init` with `go 1.26` when creating lab modules.
...
# go.mod: set `go 1.26` (or your installed version)
```
