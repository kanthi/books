...
humanize_dir_title() {
  ...
  case "$slug" in
    # Rust
    getting-started-with-rust)     printf '%s\n' "Getting Started with Rust"; return ;;
    language-basics)               printf '%s\n' "Language Basics: Variables, Types, and Expressions"; return ;;
    ownership)                     printf '%s\n' "Ownership: The Heart of Rust"; return ;;
    borrowing-and-slices)          printf '%s\n' "Borrowing, References, and Slices"; return ;;
    structs-and-enums)             printf '%s\n' "Structs, Enums, and Pattern Matching"; return ;;
    modules-and-crates)            printf '%s\n' "Modules, Crates, and Visibility"; return ;;
    collections-and-strings)       printf '%s\n' "Collections and Strings"; return ;;
    project-1-task-manager)        printf '%s\n' "Project 1: Building a CLI Task Manager"; return ;;
    error-handling)                printf '%s\n' "Error Handling in Rust"; return ;;
    generics-and-traits)           printf '%s\n' "Generics, Traits, and Associated Types"; return ;;
    lifetimes)                     printf '%s\n' "Lifetimes: Complete Understanding"; return ;;
    smart-pointers)                printf '%s\n' "Smart Pointers and Interior Mutability"; return ;;
    iterators-and-closures)        printf '%s\n' "Iterators, Closures, and Functional Patterns"; return ;;
    concurrency)                   printf '%s\n' "Concurrency: Fearless Parallelism"; return ;;
    project-3-parallel-pipeline)   printf '%s\n' "Project 3: Parallel Data Processing Pipeline with rayon"; return ;;
    async-await)                   printf '%s\n' "Asynchronous Programming with async/await"; return ;;
    macros-and-unsafe)             printf '%s\n' "Macros, Unsafe Rust, and FFI"; return ;;
    testing-and-deployment)        printf '%s\n' "Testing, Documentation, Performance, and Deployment"; return ;;
    rust-ecosystem)                printf '%s\n' "The Rust Ecosystem: Essential Crates for Production Development"; return ;;
    advanced-topics)               printf '%s\n' "Advanced Topics: Compiler, Debugging, Security, and Anti-Patterns"; return ;;
    webassembly)                   printf '%s\n' "WebAssembly with Rust"; return ;;
    embedded-rust)                 printf '%s\n' "Embedded Rust and no_std Development"; return ;;
    conclusion)                    printf '%s\n' "Conclusion: The Rust Mindset"; return ;;
    # Linux
