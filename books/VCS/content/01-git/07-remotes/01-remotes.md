...
A fork is an ordinary second remote. Git does not know about pull requests; those are a forge UI (GitHub part).

```bash
git remote add upstream https://example.com/upstream/desk-tickets.git
git fetch upstream
...
Understanding remote operations is crucial for effective team collaboration. GitHub, GitLab, Gitea, and other forges are hosting products on top of these same remote mechanics — they live in later parts of this book, not in Git itself.