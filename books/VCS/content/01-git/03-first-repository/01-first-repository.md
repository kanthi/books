...
Mental model: three trees and a DAG. Config: identity is set. This chapter is the loop you type every day — `status`, `add`, `commit`, `log`, undo — on a tiny **desk-tickets** tree.

## Walkthrough

```bash
mkdir desk-tickets && cd desk-tickets
git init
git status
```

You should see an empty branch (`main` if `init.defaultBranch` is set) and nothing to commit.

```bash
printf '%s\n' '# desk-tickets' > README.md
git add README.md
git status                 # staged
...
One commit. `git status` is clean. That is the whole product: snapshot the index, name it with a message, move the branch.

```bash
mkdir -p src
printf '%s\n' 'def ping() -> str:' '    return "ok"' > src/tickets.py
...
```bash
echo "oops" >> README.md
git restore README.md      # discard unstaged
echo "note" >> README.md
git add README.md
git restore --staged README.md    # unstage, keep the edit
git add README.md
git commit --amend --no-edit      # only if you have not pushed
```

`--amend` rewrites the last commit. Safe here; dangerous after `git push`. Commits and history covers the rest of rewriting.

The rest of this chapter is the same commands with more knobs: `add -p`, `.gitignore`, `revert`, `mv` / `rm`.
...
Daily loop: `status` → `add` (`-p` when the hunk is mixed) → `commit` → `log`. Discard with `restore`, unstage with `restore --staged`, undo a published commit with `revert`. `.gitignore` keeps build junk out of the index.

Amend and rebase of published history are the next two chapters, not this one.