...
set -euo pipefail
# ──────────────────────────────────────────────────────────────
# gen-portal.sh — Standalone portal generator
#
# Regenerates published_books/index.html and SVG covers by
# scanning the published_books/ tree.  Does NOT render any books.
#
# Usage:
#   cd books && bash scripts/gen-portal.sh
#   cd books && bash scripts/gen-portal.sh /custom/publish/dir
# ──────────────────────────────────────────────────────────────

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BOOKS_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
PUBLISH_DIR="${1:-$BOOKS_DIR/published_books}"
...
hash_hue() {
  local csum
  csum="$(printf '%s' "$1" | cksum | awk '{print $1}')"
  printf '%d' "$(((csum * 137) % 360))"
}

generate_book_cover() {
  local book_name="$1"
  local title hue cover title_len fs
...
append_book_card() {
  ...
  local title search_blob hue hue2
...
main() {
  log "gen-portal: scanning $PUBLISH_DIR"

  if [ ! -d "$PUBLISH_DIR" ]; then
    log "Error: publish directory does not exist: $PUBLISH_DIR"
    exit 1
  fi

  mkdir -p "$ASSETS_DIR" "$HTML_DIR" "$PDF_DIR" "$EPUB_DIR"

  local count=0 count_html=0 count_pdf=0 count_epub=0
  ...
  for book_path in "$HTML_DIR"/*/; do
    ...
    if [ -f "$HTML_DIR/$book_name/index.html" ] || \
       [ -n "$(find "$HTML_DIR/$book_name" -maxdepth 2 -type f -name '*.html' | head -n 1 || true)" ]; then
      has_html=1
  ...
  log "Portal complete: $count books ($count_html HTML, $count_pdf PDF, $count_epub EPUB)"
  log "Output: $PUBLISH_DIR/index.html"
}
