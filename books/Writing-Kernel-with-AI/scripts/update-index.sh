...
if [ "$BOOK_NAME" = "Writing-Kernel-with-AI" ]; then
  BOOK_TITLE="Writing Kernel with AI"
fi
...
humanize_dir_title() {
  ...
  case "$slug" in
    # Writing Kernel with AI
    parallelism-cpu-to-gpu)    printf '%s\n' "Part I: Parallelism, from the CPU up"; return ;;
    gemm-worklog)              printf '%s\n' "Part II: The GEMM Worklog: Naive to cuBLAS"; return ;;
    attention-and-profiling)   printf '%s\n' "Part III: Attention & Profiling"; return ;;
    modern-frontier)           printf '%s\n' "Part IV: The Modern Frontier"; return ;;
    ai-written-kernels)        printf '%s\n' "Part V: AI-Written Kernels"; return ;;
    capstone-projects)         printf '%s\n' "Part VI: Kernel Engineering Capstone Projects"; return ;;
    # Linux
