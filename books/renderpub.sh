...
process_book() {
  ...
  log "Rendering book with Quarto via indipub.sh (Deno heap + sequential formats)"
  # indipub.sh sets QUARTO_DENO_EXTRA_OPTIONS for Quarto 1.3 and preserves artifacts
  if ! (cd "$BOOKS_DIR" && bash "$BOOKS_DIR/indipub.sh" "$book_name"); then
