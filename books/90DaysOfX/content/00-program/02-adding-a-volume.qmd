...
Part titles come from the directory name (`03-rust` → sidebar part **Rust**).
...
3. **Expert syllabus** — stages, gates, out-of-scope, modern defaults.  
4. **Optional bridges** — 0–2 links to other volumes (never hard dependencies).  
...
| Approach | Notes |
|----------|--------|
| **Sequential** | Finish or pause volume A, then start volume B (recommended default for most people) |
| **Overlapping** | Personal choice only; this series does not define a joint timetable |
| **Pick one forever** | Perfectly valid—the series is a library of volumes |
...
- Portal book title remains **90DaysOfX** (series container)  
- A volume may later graduate to a standalone monorepo book (like existing `Go` / `NixOS`) if it outgrows the series  
